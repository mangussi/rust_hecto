...
impl Editor {
    ...
    fn draw_welcome_message(&self) {
        let mut welcome_message = format!("Hector editor -- version {VERSION}\r");
        let width = self.terminal.size().width as usize;
    ...
    fn save(&mut self) {
        ...
        self.status_message = match self.document.save() {
            Ok(()) => StatusMessage::from("File saved successfully".to_string()),
            Err(err) => StatusMessage::from(format!("Error writing file! {:?}", err)),
        };
    }
